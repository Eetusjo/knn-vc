...
def generate_morph_profile(n_frames: int, profile: str = 'linear', params: dict = None) -> Tensor:
    """
    Generate time-varying interpolation coefficients α(t) for continuous morphing.

    Arguments:
        - n_frames: Number of frames in the query sequence
        - profile: Type of interpolation curve
            * 'linear': α(t) = t/T (constant rate of change)
            * 'sigmoid': α(t) = sigmoid curve (smooth S-shaped transition)
            * 'step': α(t) = 0 before threshold, 1 after (abrupt change)
            * 'custom': user-provided array of values
    ...
    # Create normalized time vector from 0 to 1
    ...
    if profile == 'linear':
        ...
        k = params.get('steepness', 10)  # Controls transition sharpness
        alpha = torch.sigmoid(k * (t - 0.5))
        # Normalize to ensure alpha[0] ≈ 0 and alpha[-1] ≈ 1
        alpha = (alpha - alpha[0]) / (alpha[-1] - alpha[0])

    elif profile == 'step':
        # Abrupt transition at specified threshold (for control experiments)
        threshold = params.get('threshold', 0.5)
        alpha = (t >= threshold).float()

    elif profile == 'custom':
        # User provides their own interpolation curve
...
class KNeighborsVC(nn.Module):
    ...
    def match_morph(self, query_seq: Tensor,
        ...
        This method enables gradual speaker identity transition by maintaining two separate matching
        sets and blending their contributions according to a time-varying interpolation coefficient.
        At the start of the utterance (α=0), the output is 100% Speaker A. At the end (α=1), it's
        100% Speaker B. In between, it's a smooth blend of both speakers.

        Arguments:
            - query_seq: Tensor (N, dim) - source/input features from get_features()
            - matching_set_A: Tensor (N_A, dim) - reference features for Speaker A (start speaker)
            - matching_set_B: Tensor (N_B, dim) - reference features for Speaker B (end speaker)
            - synth_set_A: optional Tensor (N_A, dim) - synthesis features for Speaker A
                          (for prematched vocoder compatibility, usually same as matching_set_A)
            - synth_set_B: optional Tensor (N_B, dim) - synthesis features for Speaker B
                          (for prematched vocoder compatibility, usually same as matching_set_B)
            - topk: int - k for kNN matching (number of nearest neighbors to average)
            - morph_profile: str - type of interpolation curve ('linear', 'sigmoid', 'step', 'custom')
            - morph_params: dict - profile-specific parameters (e.g., {'steepness': 10} for sigmoid)
        ...
        # If synth sets not provided, use matching sets (standard behavior)
        ...
        # Generate morphing profile: α(t) from 0 to 1 over the utterance
        # α=0 means 100% Speaker A, α=1 means 100% Speaker B
        ...
        # Perform kNN matching against BOTH speakers independently
        # For each frame, find k nearest neighbors in each speaker's reference set
        ...
        # Get top-k nearest neighbors for each speaker
        ...
        # BLEND features according to morphing profile
        # Use broadcasting: alpha is (n_frames,) -> (n_frames, 1) for element-wise blend
        # Formula: out = (1-α) * A + α * B
        # At t=0: α≈0, so out≈A (Speaker A)
        # At t=T: α≈1, so out≈B (Speaker B)
        out_feats = (1 - alpha[:, None]) * out_feats_A + alpha[:, None] * out_feats_B

        # Vocode the blended features into waveform
        prediction = self.vocode(out_feats[None].to(device)).cpu().squeeze()
